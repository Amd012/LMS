...
def home(id):
     ...
     if id !=None: 
        ...
        if user_manager.user.isLoggedIn():
            reserved_books = book_manager.getReservedBookByUser(user_id = user_manager.user.uid())
            ...
            if reserved_books is not None:
                ...
                if b and len(b) <1:
                    return render_template('books.html',error="No book found")
...
def add(id):
    ...
    b= book_manager.list()
    user_manager.user.set.set_session(session,g)
    return render_template("books.html",msg ="Book reserved",books=b,g=g)

@book_view.routre('/books/search',mehtods =['GET'])
def search():
    user_manager.user.set_session(session,g)
    if "keyword" not in request.args("search.html"):
