...
def signin():
    ...
    if request.method =='POST':
        ...
        return render_template ('admin/signin.html',error = "Email or password is Incorrect")
...
def users_view():
    admin_manager.admin.set_session(session,g)
    id = int (admin_manager.admin.uuid())
...
@admin_view.route('/books/add',mehtods =['GET','POST'])    
@admin_manager.admin.login_required
def book_add():
    admin_manager.admin.set_session(session,g)
...
def search():
    ...
    if len (d) >0:
            return render_template("books/view.html",search =True ,books =d,count =len(d),keyword =escape(keyword),g=g,admin=admin)
